...
void serialClientPC_handler (void)
{
	...
	switch (fsmState) {
		...
		case FSM_IDLE:
            ...
            //**********************************************************************************************
			if (serialMessagePool_pendingOutputMessage(OUTPUT_INTERFACE_PC) == 1)
		...
		case FSM_MSG_SEND:
			if (stateIn)
            {
				...
				msg = serialMessagePool_peekOutputQueue(OUTPUT_INTERFACE_PC);
			...
			if (uartRB_getTxPendingBytes(&uartRB) == 0) {
				serialMessagePool_popOutputQueue(OUTPUT_INTERFACE_PC);
				gotoState(FSM_IDLE);
			}
            //**********************************************************************************************
            if (stateOut)
...
void serialClientPC_send (serialMessage_t* msg, uint8_t cmd)
{
	...
	serialMessagePool_pushOutputQueue(msg, OUTPUT_INTERFACE_PC);
}


uint32_t serialClientPC_messagePendingToRead (void)
{
	return serialMessagePool_pendingInputMessage(OUTPUT_INTERFACE_PC);
}


serialMessage_t* serialClientPC_getMessageToRead (void)
{
	return serialMessagePool_peekInputQueue(OUTPUT_INTERFACE_PC);
}


void serialClientPC_removeMessageToRead (void)
{
	serialMessagePool_popInputQueue(OUTPUT_INTERFACE_PC);	
}
...
static void parseMessageIn (uint8_t* data)
{
	...
	if (msg != NULL) {
		...
		serialMessagePool_pushInputQueue(msg, OUTPUT_INTERFACE_PC);
	}
}
